...
from waterflood_app.models.base import FitData, ModelParams, distance_mask, fit_weights
...
def fit_rolling(
    ...
    for a, b in window_bounds(grid.n_steps, cfg, breaks):
        ...
        model = CRMP(cfg)
        res = model.fit(
            FitData(wgrid, split, weights=fit_weights(wgrid, cfg), allowed=distance_mask(wgrid, cfg)),
            seed=seed,
            n_starts=3,
        )
        rep = verify(
